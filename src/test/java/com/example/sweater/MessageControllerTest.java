...
@WithUserDetails("admin") // Передаём имя пользователя под которым мы хотим выполнять данные тесты
...
public class MessageControllerTest {
    ...
    public void mainPageTest() throws Exception {
        this.mockMvc.perform(get("/main"))
                    .andDo(print())
                    .andExpect(authenticated()) // проверка авторизизрован ли пользователь. При запуске теста по умолчанию тест не пройдёт - сессия пуста
    ...
    public void addMessageToListTest() throws Exception {
        MockHttpServletRequestBuilder multipart = multipart("/main")
                ...
                .param("text", "fifth")
                .param("tag", "new one")
                .with(csrf()); // необходимая строча изза того, что мы используем csrf токены
        ...
        this.mockMvc.perform(multipart)
                    ...
                    .andExpect(xpath("//div[@id='message-list']/div[@data-id=10]/div/span").string("fifth")) // см. messages-list-before.sql, нумерация новых сообщений стартует с 10
